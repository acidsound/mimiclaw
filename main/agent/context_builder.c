...
esp_err_t context_build_system_prompt(char *buf, size_t size) {
  ...
  off += snprintf(
      ...
      "- UI loop exceptions: `ui_capture` can be called up to 4 times per turn, "
      "`ui_action` up to 6 times per turn.\n"
      ...
      "- ui_capture: ask a connected WebSocket desktop/mobile helper for a fresh "
      "screen capture (image).\n"
      "- ui_action: request one UI input action (tap/swipe/type/key) using "
      "normalized coordinates.\n"
      "- ios_sim_capture_to_telegram: for macOS+iOS Simulator testing, ask "
      "helper to open URL in Safari, tap, wait, screenshot, and send image to "
      ...
      "## Memory Persistence Policy (STRICT)\n"
      "- Write memory only for durable user facts: preferences, profile, "
      "long-term commitments, stable environment facts.\n"
      "- Do NOT store transient chat, one-off tasks, greetings, speculative "
      "guesses, or tool errors.\n"
      "- Never store secrets or credentials (tokens, passwords, API keys, "
      "cookies, private identifiers).\n"
      "- At most one memory tool call per user turn.\n"
      "- Prefer `memory_append` for day logs; use `memory_write` only when "
      "updating stable long-term memory intentionally.\n"
      "- Keep memory concise and de-duplicated. If uncertain, skip memory "
      "write.\n\n"
      "## Decision guidance\n"
      ...
      "- GUI automation loop -> call `ui_capture`, inspect the returned image, "
      "decide one `ui_action`, then call `ui_capture` again to verify.\n\n"
      "- For explicit iOS Simulator Safari capture requests, prefer "
      "`ios_sim_capture_to_telegram` with exact URL and timing.\n\n"
      "- `ui_capture`/`ui_action` require a connected WebSocket helper. If not "
      "available, ask user to connect helper first.\n\n"
