...
esp_err_t http_proxy_init(void) {
  ...
  if (nvs_open(MIMI_NVS_PROXY, NVS_READONLY, &nvs) == ESP_OK) {
    ...
    if (nvs_get_str(nvs, MIMI_NVS_KEY_PROXY_HOST, tmp, &len) == ESP_OK &&
        tmp[0]) {
...
const char *http_proxy_get_host(void) { return s_proxy_host; }

uint16_t http_proxy_get_port(void) { return s_proxy_port; }
...
static int open_connect_tunnel(const char *host, int port, int timeout_ms) {
  ...
  if (connect(sock, res->ai_addr, res->ai_addrlen) != 0) {
    ESP_LOGE(TAG, "TCP connect to proxy %s:%d failed", s_proxy_host,
             s_proxy_port);
    freeaddrinfo(res);
    close(sock);
    return -1;
