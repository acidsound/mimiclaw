...
esp_err_t tool_read_file_execute(const char *input_json, char *output,
                                 size_t output_size) {
...
esp_err_t tool_write_file_execute(const char *input_json, char *output,
                                  size_t output_size) {
  cJSON *root = cJSON_Parse(input_json);
  if (!root) {
    snprintf(output, output_size, "Error: invalid JSON input");
  ...
  FILE *f = fopen(path, "w");
  if (!f) {
    snprintf(output, output_size, "Error: cannot open file for writing: %s",
             path);
    cJSON_Delete(root);
    return ESP_FAIL;
  }
...
esp_err_t tool_edit_file_execute(const char *input_json, char *output,
                                 size_t output_size) {
  cJSON *root = cJSON_Parse(input_json);
  if (!root) {
    snprintf(output, output_size, "Error: invalid JSON input");
  ...
  if (!old_str || !new_str) {
    snprintf(output, output_size,
             "Error: missing 'old_string' or 'new_string' field");
    cJSON_Delete(root);
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (file_size <= 0 || file_size > MAX_FILE_SIZE) {
    snprintf(output, output_size, "Error: file too large or empty (%ld bytes)",
             file_size);
    fclose(f);
    cJSON_Delete(root);
  ...
  size_t max_result =
      file_size + (new_len > old_len ? new_len - old_len : 0) + 1;
  ...
  snprintf(output, output_size,
           "OK: edited %s (replaced %d bytes with %d bytes)", path,
...
esp_err_t tool_list_dir_execute(const char *input_json, char *output,
                                size_t output_size) {
  cJSON *root = cJSON_Parse(input_json);
  ...
  while ((ent = readdir(dir)) != NULL && off < output_size - 1) {
    ...
    /* Security: Only list files in the public directory */
    if (strncmp(full_path, MIMI_SPIFFS_PUBLIC_DIR,
                strlen(MIMI_SPIFFS_PUBLIC_DIR)) != 0) {
      continue;
    }

    /* Optional: further filter by user-provided prefix */
